...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Delete category function  controllers\picture_album_control\delete_picture_album.php
    function deleteCategory(categoryId) {
        $.get(`../../controllers/picture_album_control/delete_picture_album.php?id=${categoryId}`, function (response, status) {
            try {
                const data = JSON.parse(response);
                if (data.status === "success") {
                    window.location.href = "/hnd/admin/picture-album/view-album-list.php";
                } else {
