<footer class="site__footer d-flex justify-content-between align-items-center">
        <div class="fotter__icon__div">
            <i class="fa-brands fa-facebook" style="color: #fafcff;"></i>
            <i class="fa-brands fa-linkedin" style="color: #ffffff;"></i>
            <i class="fa-solid fa-phone" style="color: #ffffff;"></i>
            <i class="fa-brands fa-pinterest" style="color: #ffffff;"></i>
        </div>
        <div>
            <p class="mt-3">Malcolmphotography.com</p>
        </div>
        <div>
            <a href="/hnd/auth/index.php">
                <i class="fa-solid fa-user-tie" style="color: #ffffff;"></i>
            </a>
        </div>
</footer>